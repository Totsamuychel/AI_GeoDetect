...
(function () {
  ...
  const MOCK_MODELS = [
    { id: 'streetclip', label: 'StreetCLIP', subtitle: 'ViT-L/14 · Contrastive Learning', accuracy: '98.1%', available: true },
    { id: 'baseline', label: 'Baseline CNN', subtitle: 'EfficientNet-B2 · Softmax', accuracy: '75.6%', available: true },
    { id: 'geoclip', label: 'GeoCLIP', subtitle: 'ResNet-50 · Location Encoding', accuracy: '93.5%', available: true }
  ];
  ...
  fetch('/api/models')
    ...
    .catch(() => {
      console.log('Using mock models for static demo');
      renderModels(MOCK_MODELS);
    });
  ...
  predictBtn.addEventListener('click', () => {
    ...
    if (isStatic) {
      setTimeout(() => {
        const mockData = generateMockResult(selectedModel);
  ...
  function generateMockResult(modelId) {
    const cities = [
      { city: 'Warsaw', city_ua: 'Варшава', country: 'Польща', lat: 52.2297, lon: 21.0122 },
      { city: 'Prague', city_ua: 'Прага', country: 'Чехія', lat: 50.0755, lon: 14.4378 },
      { city: 'Budapest', city_ua: 'Будапешт', country: 'Угорщина', lat: 47.4979, lon: 19.0402 }
    ];
    // Випадковий розподіл
    let p1 = 0.7 + Math.random() * 0.25;
    let p2 = (1 - p1) * (0.4 + Math.random() * 0.5);
    let p3 = 1 - p1 - p2;
    ...
    return {
      ...
      ood: { enabled: true, is_ood: false, max_similarity: 0.85, threshold: 0.6 }
    };
  }
